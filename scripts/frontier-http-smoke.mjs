...
const child=spawn(process.execPath,[entry],{env:{...process.env,FRONTIER_DATA:dir,FRONTIER_PORT:'4187',FRONTIER_MAP_SIZE:'128'},stdio:['ignore','pipe','pipe']});let stderr='';child.stderr.on('data',c=>stderr+=c);
try{
  ...
  const first=(await request('/api/state')).body;assert.equal(first.roster.length,4);assert.equal(first.world.length,18);assert.equal(first.region.size,128);assert.ok(first.region.terrain);
...
} finally {child.kill('SIGTERM');await new Promise(resolve=>child.exitCode!==null?resolve():child.once('exit',resolve));assert.ok(dir.startsWith(path.join(tmpdir(),'frontier-http-')));await rm(dir,{recursive:true,force:true});}
