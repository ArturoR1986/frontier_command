...
const rates = new Map(), timings = []; let shuttingDown = false, ticks = 0;
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url.pathname.startsWith('/api/')) {
      const key = `${req.socket.remoteAddress}:${token || 'guest'}`, now = Date.now(), rate = rates.get(key) || { at: now, count: 0 };
      ...
      const account = authority.account(token); if (!account) return send(res, 401, { error: 'Colony access key required.' });
      if (req.method === 'GET' && url.pathname === '/api/state') return send(res, 200, authority.view(token, url.searchParams.get('region'), url.searchParams.get('terrain') !== '0'));
