...
export class Renderer {
  constructor(canvas, minimap) { this.canvas = canvas; this.ctx = canvas.getContext('2d'); this.mini = minimap; this.mx = minimap.getContext('2d'); this.camera = { x: 81, y: 128, zoom: 1 }; this.frameTimes = []; this.last = performance.now(); this.keys = new Set(); this.terrain = null; this.lastTerrain = ''; this.soil = false; this.mouse = null; }
  ...
  terrainImage(r) {
    const key = `${r.id}:${r.topology}:${this.soil}`; if (key === this.lastTerrain || !r.terrain) return;
    ...
    for (let y = 0; y < r.size; y++) for (let x = 0; x < r.size; x++) {
      ...
      g.fillStyle = this.soil && t < 2 ? `hsl(${55 + r.fertility[i] * 0.65} 24% ${20 + r.fertility[i] * 0.2}%)` : t === 0 ? `hsl(${83 + n * 9} 16% ${29 + n * 5}%)` : t === 1 ? `hsl(83 17% ${20 + n * 5}%)` : t === 2 ? `hsl(45 7% ${26 + n * 7}%)` : `hsl(174 20% ${23 + n * 5}%)`;
  ...
  draw(view, selection, build, box) {
    this.resize(); const g = this.ctx, now = performance.now(), dt = Math.min(0.1, (now - this.last) / 1000); this.last = now;
    this.frameTimes.push(dt * 1000); if (this.frameTimes.length > 600) this.frameTimes.shift();
    ...
    const known = new Set(r.explored[view.faction.id] || []), n = Math.ceil(r.size / 8); g.fillStyle = '#111b19eb';
  ...
  building(b, view, s, now, selected) {
    ...
    if (b.kind === 'field') { g.fillStyle = '#635e3c'; g.fillRect(0, 0, w, h); for (let y = 0; y < d.h; y++) for (let x = 0; x < d.w; x++) { g.fillStyle = '#353f27'; g.fillRect(x * s + s * .4, y * s, s * .2, s); g.fillStyle = b.growth >= 1 ? '#d1bc68' : '#8aa557'; const leaf = s * (.1 + .25 * b.growth); g.beginPath(); g.ellipse(x * s + s / 2, y * s + s / 2, leaf, leaf / 2, -.8, 0, 7); g.fill(); } }
      ...
      if (s > 14) { g.fillStyle = '#e5e6ce'; g.textAlign = 'center'; g.font = `${Math.min(12, s * .48)}px Segoe UI`; g.fillText(d.name, w / 2, h + 13); }
    }
  ...
  entity(e, view, s, now, selected) {
    const g = this.ctx, p = this.point(e.x, e.y), color = view.factions.find(f => f.id === e.faction)?.color || '#d19774', moving = e.route?.length > 0, bob = moving ? Math.sin(now / 90) * 1.5 : 0;
    g.save(); g.translate(p.x, p.y);
    ...
    if (s > 14 && (e.type === 'person' || selected)) { g.font = '11px Segoe UI'; g.textAlign = 'center'; g.fillStyle = '#14251bea'; const text = e.name; g.fillRect(-g.measureText(text).width / 2 - 4, s * .4, g.measureText(text).width + 8, 15); g.fillStyle = '#efe6cb'; g.fillText(text, 0, s * .4 + 11); }
    g.restore();
  ...
  drawMini(view) {
    ...
    const known = new Set(r.explored[view.faction.id] || []), n = Math.ceil(r.size / 8); g.fillStyle = '#132019d9'; for (let y = 0; y < n; y++) for (let x = 0; x < n; x++) if (!known.has(y * n + x)) g.fillRect(x * 8 * scale, y * 8 * scale, 8 * scale + 1, 8 * scale + 1);
